...
from endpoints import api_bp, ml_bp
# Import predictor so we can force model loading/logging in the server process
from endpoints.ml_backend import predictor
from services.mqtt_bridge import start_in_background
...
# Start background services immediately after creating the app
try:
    start_in_background()
except Exception as e:
    print(f"⚠️ Failed to start background services: {e}")

# Ensure the ML models are loaded in this process (prints are helpful for debugging)
try:
    predictor.load_models()
except Exception as e:
    print(f"⚠️ Error loading ML models at startup: {e}")

if __name__ == "__main__":
    # Also load models here to ensure they are initialized in the main process
    try:
        predictor.load_models()
    except Exception as e:
        print(f"⚠️ Error loading ML models before run: {e}")
